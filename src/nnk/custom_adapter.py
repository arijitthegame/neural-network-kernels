  ...
  def __init__(
        ...
        **kwargs
        ...
        if self.init_weights == 'bert':
          self.initial_weights = self.initial_weights.data.normal_(mean=0.0, std=0.02).to(self.model_device)
        ...
        **kwargs
        ...
        if self.init_weights == 'bert':
          ...
          with torch.no_grad():
              nn.init.kaiming_uniform_(self.initial_weights.data, a=math.sqrt(5))
              self.initial_weights = self.initial_weights.to(self.model_device)
        seq_list.append(NNK_Relu(input_weights=self.initial_weights, num_rfs=self.num_rfs,
                            dim=self.input_size, model_device=self.model_device, seed=self.seed,
                            normalize=self.normalize, normalization_constant=self.normalization_constant,
                            orthogonal=self.orthogonal, constant=self.constant,
                            ))
        self.adapter_down = nn.Sequential(*seq_list)
        ...
        if self.down_sample is not None :
          ...
          self.modulating_vector = torch.zeros(self.input_size) #device placement can be handled
          self.modulating_vector = nn.Parameter(self.modulating_vector)
        else :
