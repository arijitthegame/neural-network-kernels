...
class P_Measure(rv_discrete):
    ...
    def _pmf(self, k):
        if self.max_val == np.inf:
            norm_const = 1.0 / (self.p - 1.0)
        else:
            norm_const = np.sum(
                np.array([1.0 / self.p ** (n + 1.0) for n in range(self.max_val + 1)])
            )
        pmf_vals = 1.0 / (self.p ** (k + 1.0))
...
def create_random_feats(
    inputs, num_rfs, degree, coef_fun_pos, coef_fun_neg, p=2, M=1, seed=0
):
    ...
    if len(inputs.shape) == 2:
        add_features_pos = (
            torch.Tensor([coef_fun_pos(0)]).float().sqrt().repeat(len(inputs), 1)
        )
        all_features_pos = torch.cat([add_features_pos, features_pos], dim=1)
        add_features_neg = (
            torch.Tensor([coef_fun_neg(0)]).float().sqrt().repeat(len(inputs), 1)
        )
        all_features_neg = torch.cat([add_features_neg, features_neg], dim=1)

    elif len(inputs.shape) == 3:
        add_features_pos = (
            torch.Tensor([coef_fun_pos((0))])
            .float()
            .sqrt()
            .expand((features.shape[0], features.shape[1], 1))
        )
        all_features_pos = torch.cat([add_features_pos, features_pos], dim=-1)
        add_features_neg = (
            torch.Tensor([coef_fun_neg((0))])
            .float()
            .sqrt()
            .expand((features.shape[0], features.shape[1], 1))
        )
