...
def create_asym_feats(xw, AB_fun, ab_fun, xis, num_rfs, dim, device,
                                  seed, normalize, normalization_constant,
                                  orthogonal, proj_matrix, bias_term, M, is_weight):
    """
    ...
    if normalize :
      if normalization_constant is None :
        xw = torch.nn.functional.normalize(xw)
        bias_term = bias_term - bias_term.min()/ (bias_term.max() - bias_term.min()) + eps
      else :
        xw = normalization_constant * torch.nn.functional.normalize(xw)
        bias_term = normalization_constant *  bias_term - bias_term.min()/ (bias_term.max() - bias_term.min()+0.000001) # this will set bias_term to be None if input bias are zeros
    ab_coeffs = torch_apply_along_axis_asym_kernel(ab_fun, xis, 0, None, device)
    AB_coeffs = torch_apply_along_axis_asym_kernel(AB_fun, xis, 0, bias_term, device) # needs to be a vector. each entry correspond to a bias term
    ...
    if len(xw.shape) == 2 :
      ...
      dot_products = torch.einsum('ij,jk->ik', xw.type(torch.cfloat), renorm_gs.t()) #complex128 / cfloat
    elif len(xw.shape) == 3:
    ...
    if len(squared_xw.shape) == 1 :
      ...
      correction_vector = torch.einsum('pq, r->pqr', squared_xw / 2, torch.mul(ab_coeffs, ab_coeffs))
    else :
      raise ValueError("Unsupported tensor shape of xw")
...
class NNK_Asym_Kernel(nn.Module) :
  def __init__(self, input_weights, A_fun, a_fun, xis, num_rfs, dim, model_device, seed, normalize, B_fun, b_fun, M, b):
        super().__init__()
        self.input_weights = input_weights
        self.A_fun = A_fun
        self.a_fun = a_fun
        ...
        self.projection_matrix = torch.normal(mean=0, std=.02, size=(self.num_rfs, self.dim)).to(self.model_device)

        if self.normalize is False:
          self.input_weights = self.input_weights/self.dim**.25 #.5
          self.b = self.b/self.dim**.25 #.5

        self.first_rfv_plus = create_asym_feats(xw=self.input_weights, AB_fun=self.B_fun, ab_fun=self.b_fun, xis=self.xis, num_rfs=self.num_rfs
                                                        , dim=self.dim, device=self.model_device, seed=self.seed, normalize=self.normalize,
                                                   normalization_constant=None, orthogonal=False, proj_matrix=self.projection_matrix, bias_term = self.b, M=self.M,
                                                            is_weight=True)
        self.first_rfv_minus = create_asym_feats(xw=self.input_weights, AB_fun=self.B_fun, ab_fun=self.b_fun, xis=-self.xis, num_rfs=self.num_rfs
                                                        , dim=self.dim, device=self.model_device, seed=self.seed, normalize=self.normalize,
                                                   normalization_constant=None, orthogonal=False, proj_matrix=self.projection_matrix, bias_term = self.b, M=self.M,
                                                             is_weight=True)
        self.weights = (1.0 / np.sqrt(2.0)) * np.power(1.0 + 4.0 * self.M, self.dim / 4.0) * torch.cat(
  ...
  def forward(self, x):
        ...
        x_rfv_plus = create_asym_feats(xw=x, AB_fun=self.A_fun, ab_fun=self.a_fun, xis=self.xis, num_rfs=self.num_rfs
                                                        , dim=self.dim, device=self.model_device, seed=self.seed, normalize=self.normalize,
                                                   normalization_constant=None, orthogonal=False, proj_matrix=self.projection_matrix, bias_term = self.b, M=self.M,
                                                   is_weight=False)
        x_rfv_minus = create_asym_feats(xw=x, AB_fun=self.A_fun, ab_fun=self.a_fun, xis=-self.xis, num_rfs=self.num_rfs
                                                        , dim=self.dim, device=self.model_device, seed=self.seed, normalize=self.normalize,
                                                   normalization_constant=None, orthogonal=False, proj_matrix=self.projection_matrix, bias_term = self.b, M=self.M,
                                                    is_weight=False)
        output_x = (1.0 / np.sqrt(2.0)) * np.power(1.0 + 4.0 * self.M, self.dim / 4.0) * torch.cat([x_rfv_plus, x_rfv_minus], dim=-1)
