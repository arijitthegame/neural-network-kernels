...
class ReluAdapterViTLayer(nn.Module):
    ...
    def __init__(self, config, num_rfs,
        ...
        self.output = ReluAdapterViTOutput(config=self.config, num_rfs=self.num_rfs,
                                             model_device=self.model_device, seed=self.seed,
                                             down_sample=self.down_sample, init_weights=self.init_weights,
                                             constant=self.constant, normalize=self.normalize,
                                             normalization_constant=self.normalization_constant,
                                             orthogonal=self.orthogonal,
                                             )
        self.layernorm_before = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.layernorm_after = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.attention_adapters = CustomReluAdapter(input_size=self.config.hidden_size, num_rfs=self.num_rfs,
                                                model_device=self.model_device,
                                                seed=self.seed,
                                                down_sample = self.down_sample,
                                                init_weights = self.init_weights,
...
class CustomAdapterViTEncoder(ViTEncoder):
    ...
    def __init__(self, config, num_rfs,
      ...
      self.layer = nn.ModuleList([ReluAdapterViTLayer(config=self.config, num_rfs=self.num_rfs,
                                                model_device=self.model_device,
                                                seed=self.seed,
                                                down_sample = self.down_sample,
                                                init_weights = self.init_weights,
                                                        ...
                                                        normalize = self.normalize,
                                                        normalization_constant = self.normalization_constant,
                                                        orthogonal = self.orthogonal) for _ in range(config.num_hidden_layers)])
...
class ReluAdapterViTModel(ViTModel):
    def __init__(self, config, num_rfs,
        ...
        self.encoder = ReluAdapterViTEncoder(config=self.config, num_rfs=self.num_rfs,
                                                model_device=self.model_device,
                                                seed=self.seed,
                                                down_sample = self.down_sample,
                                                init_weights = self.init_weights,
...
class ReluAdapterViTForImageClassification(ViTForImageClassification):
    def __init__(self, config, num_rfs,
        ...
        self.vit = ReluAdapterViTModel(config=config,
